...
Итоговый размер образа — цифра, с которой уже путались один раз, поэтому здесь написано, какая команда какое число даёт. Замер: `docker build .` на HEAD `feat/embedded-s3`, arm64, Docker Desktop с containerd-снэпшоттером, 2026-08-21, Dockerfile с боевыми пинами (не с локально подменённым web-ui):

| Откуда | Число |
|---|---|
| `docker images`, колонка DISK USAGE | ~929 МБ |
| `docker images`, колонка CONTENT SIZE | ~236 МБ |
| `docker image inspect --format '{{.Size}}'` | ~225 МБ |

Это одна и та же сборка, посчитанная тремя способами. Почему CONTENT SIZE и `.Size` расходятся на 11 МБ — не выяснено; важно, что это разные числа и подставлять одно вместо другого нельзя. Старая цифра ~218 МБ в этом файле, судя по порядку величины, относилась к одной из двух правых строк таблицы на другом коммите — отсюда и четырёхкратный разрыв с тем, что видно в самой заметной колонке (DISK USAGE) и что ближе к ощущениям от `docker pull`. Колонок две только на движке с containerd-снэпшоттером; без него `docker images` печатает одну.

Прирост, который добавил конкретно embedded S3 (бинарник versitygw + конфиги), **не измерен**. Для честной дельты нужна сборка того же коммита без S3-стейджа в идентичных условиях; сравнивать с более старыми локальными тегами нельзя — они отличаются от HEAD не только S3.
...
Бакет для versitygw — это просто директория внутри `$STORAGE_DIR`: posix-бэкенд отдаёт в `ListBuckets` то, что там физически есть. Никакого отдельного API создания бакетов нет — `run`-скрипт сам делает `mkdir -p` для четырёх директорий (`vault`, `posters`, `subtitles`, `thumbnails`), уже после того, как проверил, что хранилище доступно на запись и поддерживает extended attributes. На сегодня `vault` никем не читается — зарезервирован под ещё не влитый сервис vault (следующий суб-проект, см. `.superpowers/sdd/2026-08-20-embedded-s3/`); `posters`/`subtitles`/`thumbnails` уже используются web-ui через `AWS_*_BUCKET` в `common.template.env`.
